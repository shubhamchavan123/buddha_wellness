...
// dashboard_screen.dart
import 'package:buddha_wellness/utils/BaseScreen.dart';
import 'package:flutter/material.dart';
...
class _DashboardScreenState extends State<DashboardScreen> with WidgetsBindingObserver {
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) {
      // App came back to foreground, ensure bottom nav is visible
      if (!_showBottomNav) {
  ...
  Widget build(BuildContext context) {
    return BaseScreen(
      child: Scaffold(
        body: _selectedIndex == 3 // Profile screen is at index 3
            ? _screens[_selectedIndex] // Don't apply scroll listener for profile
        ...
        // 👇 Always show bottom nav for profile screen, hide/show for others
        bottomNavigationBar: AnimatedContainer(
          duration: const Duration(milliseconds: 300),
          height: _selectedIndex == 3 ? kBottomNavigationBarHeight : (_showBottomNav ? kBottomNavigationBarHeight : 0),
          child: Wrap(
            children: [
              BottomNavigationBar(
                ...
                onTap: (index) {
                  setState(() {
                    ...
                    // Ensure bottom nav is visible when switching to profile
                    if (index == 3) {
                      _showBottomNav = true;
                    }
                  });
